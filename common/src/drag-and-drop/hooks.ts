...
export const useClickToDragDraggable = (rawDraggableId: string, droppableId?: string) => {
  const { selectedDraggableId, dragPreviewPosition, onDraggableClick } = useContext(ClickToDragContext);
  return { selectedDraggableId, dragPreviewPosition, onClick: onDraggableClick.bind(null, rawDraggableId, droppableId) };
};
