...
export interface IUseModelStateResult<IModelInputState, IModelOutputState> {
  ...
  isFinished: boolean;
  setInputState: (update: Partial<IModelInputState>, overrideFinishedState?: boolean) => void;
...
export const useModelState = <IModelInputState, IModelOutputState>(
  ...
  const setInputState = useCallback((update: Partial<IModelInputState>, overrideFinishedState?: boolean) => {
    ...
    setModelRuns(oldState => {
      if (oldState[activeRunIdx].isFinished && !overrideFinishedState) {
  ...
  const resetModelRun = () => {
    ...
    setModelRuns(oldState => {
      const newState = [...oldState];
      newState[activeRunIdx] = getNewModelRun();
      return newState;
    });
  };
