...
module.exports = (env, argv, simulationDirname, customizations) => {
  ...
  const common = {
    ...
    plugins: [
      ...
      // Generate Spanish package in production mode and zip English and Spanish directories.
      ...(!devMode ? [new FileManagerPlugin({
        events: {
          onEnd: [
            // Make a copy of English package and add -es suffix.
            { copy: [{source: `${simPath}/`, destination: `${simPath}-es/`}]},
            // Remove Spanish files from English package to avoid confusion and the opposite from the Spanish package.
            { delete: [
              {source: `${simPath}/index-es.html`, options: { force: true}},
              {source: `${simPath}-es/index.html`, options: { force: true}},
              {source: `${simPath}/index-top-es.html`, options: { force: true}},
              {source: `${simPath}-es/index-top.html`, options: { force: true}},
            ]},
            // Makes Spanish files default ones in -es package.
            { move: [
              {source: `${simPath}-es/index-es.html`, destination: `${simPath}-es/index.html`},
              {source: `${simPath}-es/index-top-es.html`, destination: `${simPath}-es/index-top.html`}
            ]},
            // Zip both packages.
            {archive: [
              {source: simPath, destination: `${simPath}${zipExtension}.zip`},
              {source: `${simPath}-es`, destination: `${simPath}-es${zipExtension}.zip`}
            ]},
          ],
        },
      })] : [])
